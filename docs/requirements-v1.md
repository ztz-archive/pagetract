# ScanDoc2MD — 基于混合架构的高精度扫描件文档转 Markdown 系统
...
ScanDoc2MD 是一个开源的文档处理工具，专注于将**扫描件 PDF**（整页都是图片、无文本层）高精度地转换为带图片引用的结构化 Markdown 文档。
...
```bash
# 最简用法
scandoc2md input.pdf -o output/

# 指定配置文件
scandoc2md input.pdf -o output/ --config my_config.yaml
...
# 批量处理目录下所有 PDF
scandoc2md ./pdfs/ -o output/ --batch

# 仅做布局检测（调试用，不调用 VLM）
scandoc2md input.pdf -o output/ --layout-only

# 仅处理指定类型的区域
scandoc2md input.pdf -o output/ --only-types text,table,image
```
...
```python
from scandoc2md import PDFRenderer, LayoutDetector, VLMEngine, MarkdownAssembler
...
2. **MCP Server 集成**：将 ScanDoc2MD 封装为 MCP 工具，供 Claude Code 等 AI agent 调用
