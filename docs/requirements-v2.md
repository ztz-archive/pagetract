...
# 指定配置文件
pagetract convert input.pdf -o output/ --config my_config.yaml
...
# 强制指定 PDF 处理模式
pagetract convert input.pdf -o output/ --force-mode scanned

# 批量处理目录下所有 PDF
pagetract convert ./pdfs/ -o output/ --batch

# 仅做布局检测（调试用，不调用 VLM）
pagetract convert input.pdf -o output/ --layout-only

# 仅处理指定类型的区域
pagetract convert input.pdf -o output/ --only-types text,table,image
```
...
# 指定端口
pagetract serve --port 9000

# 使用自定义配置
pagetract serve --config production.yaml
```
...
```python
from pagetract import PDFRenderer, PDFTypeDetector, LayoutDetector, VLMEngine, MarkdownAssembler
...
1. **MCP Server 集成**：将 pagetract 封装为 MCP 工具，供 Claude Code 等 AI agent 调用
