...
提供可视化前端，用于快速体验 ScanDoc2MD 的转换效果、项目演示和测试。
...
示例：`scandoc2md convert file.pdf --model gpt-4o` 覆盖 config.yaml 中的 `vlm.model`。
...
```bash
scandoc2md convert    # 转换 PDF
scandoc2md serve      # 启动 API 服务
scandoc2md config     # 配置管理 (init/show/validate)
scandoc2md doctor     # 检查环境和依赖
scandoc2md version    # 版本信息
```
...
# 指定配置 + 覆盖参数
scandoc2md convert input.pdf -o output/ --config my_config.yaml --model gpt-4o --dpi 400

# 指定页码范围 + 强制模式
scandoc2md convert input.pdf -o output/ --pages 1,2,3,5-10 --force-mode scanned

# 批量处理（递归子目录）
scandoc2md convert ./pdfs/ -o output/ --batch --recursive

# 成本预估（dry-run，不调用 VLM）
scandoc2md convert input.pdf --dry-run

# 仅做布局检测（调试用）
scandoc2md convert input.pdf -o output/ --layout-only

# JSON 结构化输出（便于脚本集成）
scandoc2md convert input.pdf -o output/ --output-format json

# 指定最大预算
scandoc2md convert input.pdf -o output/ --max-cost 5.0

# 使用/不使用缓存
scandoc2md convert input.pdf -o output/ --no-cache

# 日志级别
scandoc2md convert input.pdf -o output/ --log-level debug --log-file convert.log
```

### 8.3 服务启动

```bash
scandoc2md serve --port 9000 --config production.yaml
```

### 8.4 配置管理

```bash
scandoc2md config init     # 交互式初始化配置文件
scandoc2md config show     # 显示当前生效的配置
scandoc2md config validate # 校验配置合法性
```
...
converter = ScanDoc2MD(config_path="config.yaml")
...
# 快速 Demo (Gradio)
scandoc2md demo  # 或 python -m scandoc2md.demo
